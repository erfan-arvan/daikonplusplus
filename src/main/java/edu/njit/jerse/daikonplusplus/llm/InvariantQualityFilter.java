...
public final class InvariantQualityFilter {
  ...
  public static boolean keep(
    ...
    // 4) Must reference at least one in-scope name (wrap keySet for CF compatibility)
    if (!mentionsAny(e, new ArrayList<>(inScope.keySet()))) return false;

    // 5) For EXIT with non-void methods (caller supplies 'result' in scope), require 'result'
    if (isExit && inScope.containsKey("result") && !containsWord(e, "result")) return false;

    // 6) Reject null-comparisons on primitives (e.g., "result != null" when result is int/boolean)
    {
